...
for j in range(2):
    for i in range(3):
        ...
        axs[j, i].text(0.95, 0.95, labels[3* j + i], fontsize=14, color='white', transform=axs[j, i].transAxes, ha='center', va='center')
        axs[j, i].axis('off')

# Add one more axis and display the descriptions
right_axis = fig.add_axes([0.72, 0.1, 0.02, 0.8])
right_axis.axis('off')

for i, description in enumerate(descriptions):
    right_axis.text(0.1, 0.66 - i * 0.06, description, transform=right_axis.transAxes, fontsize=10, va='top', ha='left')
...
show_image(image_blown_ic_8bit - reconstructed_cropped_image_8bit, vmin=0, vmax=255, title='Difference of the Images')
print(f'The absolute mean difference between the input image and the final image:\n{np.mean(np.abs(image_blown_ic - reconstructed_cropped_image))}')